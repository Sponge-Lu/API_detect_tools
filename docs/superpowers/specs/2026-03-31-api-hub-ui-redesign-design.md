...
- 产品需支持 `3 种不同浅色主题 + 1 个统一暗色主题`，但主题切换不能改变信息结构、组件层级或操作路径。
...
- 当前主题切换能力仍只覆盖 `light / dark / system` 三种模式，尚未区分多套浅色主题，见 [useTheme.ts](/D:/2_Github_Repository/API_detect_tools/src/renderer/hooks/useTheme.ts#L20)。
...
- 四套主题共享同一套语义 token 结构，不为某个主题单独发明组件规则。
- 四套主题共享相同的信息架构、页面原型、组件层级、圆角体系、阴影逻辑与 overlay 规则。
- 主题切换只改变色相倾向、明度关系与气质，不改变列表密度、列组关系、按钮位置和操作路径。
- 所有主题都必须继续遵守 `低饱和 / 无渐变 / 少边框 / 靠明度差建立层级` 的总原则。

#### 4.4.1 Shared Semantic Tokens
...
- 主题实现时允许数值不同，但语义位置必须一致。
- 不允许某个主题通过额外描边、额外高光或额外背景层来“补气质”。

#### 4.4.2 Light A

`Light A` 作为默认推荐浅色主题，基调为：
...
它对应当前已确认的 `高级克制` 主方向，最接近用户已选的 `A 为主，吸收 B/C 某些部分`。

#### 4.4.3 Light B

`Light B` 保持同样克制，但转向更冷静的矿物感：

- 冷灰或蓝灰背景
- 更清晰的明度压差
- 低饱和钢蓝 / 青灰强调

它吸收前序方向里的理性、技术感和秩序感，适合更偏“工具台”的阅读氛围。
...
它用于提供第三种浅色选择，但不能变成“普通白底商务后台”。它仍需保留纸面感和克制的层级关系。

#### 4.4.5 Unified Dark Theme

暗色主题只保留一套统一方案，不做多分支暗色变体。

定义要求：

- 以石墨、炭灰、深板岩为基础。
- 通过层层加亮的表面关系建立结构，而不是通过霓虹色强调。
- 强调色仍保持低饱和，不引入高纯度蓝紫。
- 站点高密列表在暗色下仍需清晰可扫，不能因为压暗而牺牲可读性。

#### 4.4.6 Hard Constraints

- 不允许为不同主题制作不同布局版本。
- 不允许某个浅色主题使用渐变，而另一个主题不用。
- 不允许暗色主题成为一套独立语言系统。
- 不允许主题差异影响弹窗、抽屉、右键菜单和固定排序条的结构一致性。
...
### 10.2 Cross-Theme Consistency

overlay 在四套主题中的变化只允许发生在表面色、文字对比和遮罩明度上。
...
换言之，`编辑站点` 不应该因为主题不同而像不同组件；它只能是同一家族在不同色调下的表现。
...
- CLI 任务域独立，认知边界清楚。
- 弹窗体系统一，整体感显著增强。
- 用户可以在 `3 种浅色主题 + 1 个统一暗色主题` 之间切换，而不需要重新学习界面。
...
- 只允许主题改变色调家族，不允许改变结构语言。
- 所有主题共享同一套语义 token、组件层级和 overlay family。
