...
- **独立自动刷新**：每个站点可单独开启/关闭自动刷新功能
  - 点击站点卡片上的刷新按钮切换自动刷新开关
  - 开启时弹出设置对话框，默认间隔 30 分钟（最小 15 分钟），用户可调整后确认
  - 关闭时直接禁用，无需弹窗
...
- **并发可调**：设置中可开启/关闭并发并设置最大并发数（默认 3，1–5 可调），根据机器和网络情况权衡速度与稳定性。多站点依次单独刷新时，每个站点独立显示加载状态。
...
- **浏览器模式优化**：
  ...
  - 多站点并发刷新时，浏览器页面互不干扰，共享页面失效时自动重试。
...
- **应用内下载与安装（v2.1.18+）**：
  ...
  - 下载使用 Chromium 网络栈，自动继承系统代理设置，网络错误时自动重试（v2.1.23+）。
