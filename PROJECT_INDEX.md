...
| `src/renderer/components/dialogs/AddAccessPointDialog.tsx` / `AccessPointDetailPanel.tsx` / `OperationRecordDialog.tsx` / `CliProbeSettingsDialog.tsx` | 统一添加接入点弹窗、接入点详情侧滑面板、操作记录弹窗与站点 CLI 探测设置弹窗；托管站点和直连配置的 CLI 编辑内容均覆盖 Claude Code、Codex、OpenCode、Grok Build，Grok Build 支持模型/上游协议/预览/合并应用并明确标记“暂不支持探测”；探测设置弹窗只调度当前具备执行器的三种 CLI |
