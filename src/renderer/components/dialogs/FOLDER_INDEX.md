...
| **ManagedCliConfigEditorContent.tsx** | 托管站点 CLI 配置编辑内容 | 四种内置 CLI 卡片常驻展开，紧凑配置 API Key、使用模型与上游协议，配置预览按需折叠 |
| **DirectCliConfigEditorContent.tsx** | 直连配置编辑内容 | 四种内置 CLI 卡片常驻展开，紧凑展示 API Key、使用模型与上游协议，配置预览按需折叠 |
...
| **AccessPointDetailPanel.tsx** | 接入点详情侧滑面板 | `AccessPointDetailPanel` 组件，承载信息、模型 & 资源、CLI 配置与独立测试四页 |
| **EndpointTestPanel.tsx** | 端点测试面板 | 三卡常驻展开；托管模型按 Key 分组过滤并可列出全部，直连列出全部模型，显示简短结果和最近测试时间 |
...
| **SiteSettingsDialog.tsx** | 站点设置弹窗 | 只维护站点刷新参数，不包含定时测试 |
