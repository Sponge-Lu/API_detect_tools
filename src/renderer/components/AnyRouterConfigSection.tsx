...
interface AnyRouterConfigSectionProps {
  ...
  accountId: string;
  userHash: string;
  onUserHashChange: (hash: string) => void;
  onConfigChanged?: () => void; // 新增：配置变更回调
}
      ...
      <div className="rounded-[var(--radius-lg)] border border-[var(--line-soft)] bg-[var(--surface-2)] px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 flex items-center gap-2 justify-end">
            <input
              ...
              disabled={extracting}
              className="flex-1 rounded-[var(--radius-sm)] border border-[var(--line-soft)] bg-[var(--surface-1)] px-2 py-1 font-mono text-sm text-[var(--text-primary)] placeholder-[var(--text-tertiary)] placeholder:text-xs outline-none"
            />
            <button
              type="button"
            ...
            <button
              ...
              disabled={extracting}
              className="whitespace-nowrap px-2 text-xs font-medium text-[var(--accent)] transition-colors hover:text-[var(--accent-strong)] disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {extracting ? '⏳ 提取中' : '🔄 自动提取'}
            </button>
          </div>
        ...
        {extracting && (
          <div className="mt-2 rounded-[var(--radius-sm)] border border-[var(--warning)]/30 bg-[var(--warning-soft)] p-2 text-xs text-[var(--warning)]">
            ...
            <div className="mt-1">
              ⚠️ 提取过程中会临时修改 Claude Code 配置，请勿使用 Claude Code，完成后会自动恢复。
            </div>
          </div>
        )}
...
claude`}
            ...
            <div className="rounded-[var(--radius-sm)] border border-[var(--warning)]/30 bg-[var(--warning-soft)] p-2">
              <p className="font-semibold text-[var(--warning)]">⚠️ 安全提示</p>
