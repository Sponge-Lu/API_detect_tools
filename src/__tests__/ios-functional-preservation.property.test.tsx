...
import { AppInput } from '../renderer/components/IOSInput';
...
describe('UI functional preservation contracts', () => {
  ...
  beforeEach(() => {
    ...
    styleElement.textContent = `
      :root {
        ...
        /* legacy compatibility tokens still referenced by passive primitives */
        --ios-blue: var(--accent);
        --ios-red: var(--danger);
  ...
  it('renders IOSCard title attributes and child content exactly as provided', () => {
    fc.assert(
      fc.property(
        fc.stringMatching(/^[a-zA-Z0-9]+$/).filter(value => value.length >= 1 && value.length <= 50),
  ...
  it('preserves controlled AppInput values', () => {
    fc.assert(
      fc.property(fc.stringMatching(/^[a-zA-Z0-9]*$/).filter(value => value.length <= 50), inputValue => {
        const { container, unmount } = render(
  ...
  it('renders AppModal title and content correctly through the neutral public primitive', () => {
    fc.assert(
      fc.property(
        fc.stringMatching(/^[a-zA-Z0-9 ]+$/).filter(value => value.trim().length >= 1 && value.length <= 50),
        fc.stringMatching(/^[a-zA-Z0-9 ]+$/).filter(value => value.trim().length >= 1 && value.length <= 80),
        (title, content) => {
  ...
  it('preserves AppInput error-state rendering', () => {
    fc.assert(
      fc.property(fc.stringMatching(/^[a-zA-Z0-9]+$/).filter(value => value.length >= 5 && value.length <= 50), errorMessage => {
        const { container, unmount } = render(
  ...
  it('preserves local state propagation through AppInput updates', () => {
    fc.assert(
      fc.property(
        ...
        fc.stringMatching(/^[a-zA-Z0-9]+$/).filter(value => value.length >= 1 && value.length <= 20),
  ...
  it('preserves IOSCard expanded-state reflection when props change', () => {
    fc.assert(
      fc.property(fc.boolean(), initialExpanded => {
        const { container, rerender, unmount } = render(
          <IOSCard expanded={initialExpanded} expandContent={<div>Expanded content</div>}>
  ...
  it('preserves form submission handlers built from AppInput and AppButton', () => {
    fc.assert(
      fc.property(
        fc.record({
          name: fc.stringMatching(/^[a-zA-Z]+$/).filter(value => value.length >= 1 && value.length <= 10),
