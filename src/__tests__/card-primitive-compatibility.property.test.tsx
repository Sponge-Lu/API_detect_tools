/**
 * @file src/__tests__/card-primitive-compatibility.property.test.tsx
 * @description 卡片原语兼容属性测试
 *
 * Feature: card-primitive-compatibility
 *
 * 测试属性:
...
describe('Card Primitive Compatibility - Property Tests', () => {
  let styleElement: HTMLStyleElement;

  beforeEach(() => {
    // Inject neutral theme variables for card primitive tests
  ...
  // Feature: card-primitive-compatibility, Property 2: Backdrop Blur Application
          ...
          (variant, blur) => {
            ...
            if (card) {
              ...
              if (blur) {
                ...
                // Verify the blur value stays within the supported primitive range (8px to 40px)
  ...
  // Feature: card-primitive-compatibility, Property 3: Multi-layer Shadow System
    ...
    it('should apply multi-layer shadow for standard and elevated variants', () => {
      fc.assert(
        fc.property(fc.constantFrom('standard', 'elevated'), variant => {
          const { container } = render(
            <AppCard variant={variant as 'standard' | 'elevated'}>
              <div>Test Content</div>
            </AppCard>
          );
    ...
    it('should have lighter shadow for grouped variant', () => {
      fc.assert(
        fc.property(fc.constant('grouped'), variant => {
          const { container } = render(
            <AppCard variant={variant as 'grouped'}>
              <div>Test Content</div>
            </AppCard>
          );
  ...
  // Feature: card-primitive-compatibility, Property 9: Card Expand Transition
    ...
    it('should set maxHeight to 0 when collapsed and positive when expanded', () => {
      fc.assert(
        fc.property(fc.boolean(), expanded => {
          ...
          const { container } = render(
            <AppCard expanded={expanded} expandContent={expandContent}>
              <div>Main Content</div>
            </AppCard>
          );
  ...
  // Additional tests for the card primitive compatibility layer

  describe('Card Primitive Border Radius', () => {
    it('should use the standard primitive border radius (16px)', () => {
      fc.assert(
        fc.property(fc.constantFrom('standard', 'elevated', 'grouped'), variant => {
          const { container } = render(
            <AppCard variant={variant as 'standard' | 'elevated' | 'grouped'}>
          ...
          if (card) {
            const classList = card.className;
            // Should have the shared primitive border radius
  ...
  describe('Card Primitive Disabled State', () => {
    it('should apply disabled styles when disabled', () => {
      fc.assert(
        fc.property(
          ...
          (variant, disabled) => {
            const { container } = render(
              <AppCard variant={variant as 'standard' | 'elevated' | 'grouped'} disabled={disabled}>
                <div>Test Content</div>
              </AppCard>
            );
  ...
  describe('Card Primitive Drag Over State', () => {
        ...
        fc.property(
          ...
          (variant, isDragOver) => {
            ...
            if (card) {
              ...
              if (isDragOver) {
                expect(classList).toContain('border-[var(--accent)]');
...
// ========== AppCard Sub-components Tests ==========

describe('Card Primitive Sub-components', () => {
  let styleElement: HTMLStyleElement;
  ...
  describe('CardDivider Primitive', () => {
    it('should render a divider with the shared separator token', () => {
      fc.assert(
        fc.property(fc.constant(true), () => {
          const { container } = render(<AppCardDivider />);
