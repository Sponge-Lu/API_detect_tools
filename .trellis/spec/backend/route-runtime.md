...
| Non-streaming upstream returns zero-usage `HTTP 200` with fallback available | upstream status is `200`, response is not streamed, primary usage token fields are present, and every observed usage/cache token field is `0` | Record `statusCode: 200` as `failure` with `empty_response_zero_usage`, update route-path failure state for the concrete `resolvedModel`, and try the next enabled candidate, including sibling `resolvedModel` entries under the same API key |
| Non-streaming upstream returns zero-usage `HTTP 200` with no fallback | same zero-usage validation, but no later enabled candidate exists | Do not pass the upstream `200` body to the CLI; return aggregate proxy failure and keep the path failure sample for disable/rate-limit decisions |
...
- Good: a non-streaming Codex/OpenAI Responses `HTTP 200` body with explicit `input_tokens/output_tokens/total_tokens/cache_* = 0` is recorded as `empty_response_zero_usage`; if the same API key also covers another `resolvedModel`, the proxy tries that sibling model before replying to the CLI.
- Good: when the last enabled non-streaming candidate returns explicit all-zero usage with `HTTP 200`, the proxy returns aggregate failure instead of a misleading successful empty body.
...
- Bad: treating non-streaming `HTTP 200` plus explicit all-zero generation usage as success, because the route can repeat the same empty path forever and never reaches sibling original-model candidates under the same API key.
...
- `src/__tests__/route-proxy-service.test.ts`: assert a non-streaming `HTTP 200` response with explicit all-zero total/input/output/cache usage records `empty_response_zero_usage` as a route-path failure and falls back to another `resolvedModel` under the same API key.
- `src/__tests__/route-proxy-service.test.ts`: assert a non-streaming all-zero-usage `HTTP 200` is not returned to the CLI when no fallback candidate remains.
