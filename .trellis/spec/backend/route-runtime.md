...
- A successful transparent streaming candidate must have an initial body chunk that looks like SSE (`event:`, `data:`, `id:`, `retry:`, or an SSE comment). Obvious HTML, JSON, empty, or non-SSE 2xx responses must be treated as `invalid_streaming_response` before any downstream bytes are written. An empty successful SSE body is retried once against the same route path before path failure/fallback is recorded; HTML, JSON, and non-SSE bodies are not retried.
- Claude Code and Codex transparent SSE streams must not silently end on upstream EOF unless a protocol terminal marker has been observed (`message_stop` for Anthropic/Claude Code; `response.completed`, `response.incomplete`, `response.failed`, or `[DONE]` for OpenAI Responses/Codex). An upstream `event: error`, payload-level error, or `response.failed` event must be forwarded unchanged and recorded as `upstream_streaming_error:<type-or-code>` without appending a second local error event. Only EOF with neither a success/incomplete marker nor an explicit failure event is recorded as `incomplete_streaming_response:missing_terminal_event` and receives the protocol-shaped local error.
...
- Selected CLI model routing must keep the selected CLI type, `canonicalModel`, `targetProtocol`,
  ...
  bug fix to one rule. Route-path state, in-memory first-hit logs, successful-path affinity, and
  reset matching must not promote or preserve a site/API-key path from another canonical/display
  item merely because it shares the same site, account, API key, upstream model name, route rule, or
  recent success timestamp. For example, selecting Claude Code `GLM5.2` must not keep routing to a
  previously successful ElySiver path after the user changes priority or resets the priority hit
...
- Priority-hit reset from the redirection UI is channel-scoped across all route rules for the
  ...
  route-rule/resolved-model state for the same visible site/API-key hit, so changing site priority
...
- `handleRequest()` must treat a non-streaming upstream `HTTP 200` response with explicit primary usage token fields and all total/input/output/cache-write/cache-read/cached token values equal to `0` as a model semantic anomaly, not route success. Before recording a route-path failure, retry the same active channel with a small bounded retry budget (`ZERO_USAGE_UPSTREAM_RETRY_ATTEMPTS`). If the retry returns normal non-zero usage, record only the successful final response for that route path and return it. If the retry budget is exhausted and the response is still all-zero, record the request/path with `statusCode: 200`, `outcome: 'failure'`, and `error: 'empty_response_zero_usage'`, then apply the normal route-path failure window/disable rules and try the next enabled candidate before any downstream response is written. If no fallback candidate remains, respond with the aggregate proxy failure instead of returning the upstream `200` body.
- A route path's concrete identity includes `resolvedModel`; when one API key covers multiple original/upstream models, a zero-usage `HTTP 200` for the first `resolvedModel` must not consume or suppress sibling `resolvedModel` candidates under the same site/account/API key. Those sibling models remain eligible fallback targets unless their own concrete path state is disabled.
...
- `resetRoutePathStates(params?)` must clear only `routing.routePathStates`, never `routing.stats`.
  When `params.canonicalModel` is provided, clear every path state for that redirection card's
  canonical model across all route rules unless `params.routeRuleId` is explicitly provided. When
  `params.routeRuleId`, `siteId`, `accountId`, `apiKeyId`, `resolvedModel`, or `targetProtocol` are
  also provided, every provided filter must match before a path state is deleted. Priority-hit reset
  must not provide `routeRuleId`; it is intended to clear the visible site/API-key hit for the
  selected model across all matching route rules.
...
- The redirection detail action bar must expose `重置优先命中` before the `恢复路径` button when the
  ...
  `resetRoutePathStates()` with the visible route channel identity: `canonicalModel`, `siteId`,
  `accountId`, `apiKeyId`, and `targetProtocol` when present. It must not include `routeRuleId` or
  `resolvedModel`, because one visible priority hit can have several concrete route rules and
  upstream model variants under the same site/API-key channel. This clears the successful-path
  affinity for that route channel across all matching route rules and must not reset other API keys
  under the same redirection card. The reset must leave short-lived route path states with
  `affinitySuppressedUntil` for each cleared concrete path plus a same-channel
  `routeRuleId: undefined` / `resolvedModel: undefined` suppression state, even if no current state
  was deleted, so an in-flight CLI stream that reports success again cannot immediately restore the
  same priority hit; the next request should follow normal site priority order and start from the first site.
...
| Transparent stream ends without terminal marker | downstream bytes were already written, but no protocol success/incomplete marker and no explicit upstream failure event was observed before upstream EOF | Append SSE `event: error`, record `incomplete_streaming_response:missing_terminal_event`, mark the path failed, and close the downstream response instead of silently ending |
| Transparent stream emits an upstream failure event | Anthropic/OpenAI stream contains `event: error`, a payload-level error, or `response.failed` after downstream streaming starts | Preserve the upstream event bytes, record `upstream_streaming_error:<type-or-code>`, mark the path failed, and close without appending a second local missing-terminal error |
...
| Codex stream finishes with a non-message Responses output item | stream emits a typed `ResponseOutputItem` such as `custom_tool_call`, `local_shell_call`, `computer_call`, reasoning, or MCP output, then `response.completed` / `response.incomplete` with non-zero or unknown usage | Forward the terminal event unchanged and treat the stream as consumable output; do not inject `malformed_streaming_response:empty_response` |
...
| First non-streaming zero-usage `HTTP 200`, retry succeeds | upstream status is `200`, response is not streamed, primary usage token fields are present, every observed usage/cache token field is `0`, then the same active channel retry returns normal usage | Record only the successful final response/path outcome; do not record `empty_response_zero_usage`, do not suppress affinity, and do not try fallback candidates |
| Non-streaming upstream returns zero-usage `HTTP 200` with fallback available after retry | same zero-usage validation persists after the bounded same-channel retry budget is exhausted | Record `statusCode: 200` as `failure` with `empty_response_zero_usage`, update route-path failure state for the concrete `resolvedModel`, and try the next enabled candidate, including sibling `resolvedModel` entries under the same API key |
| Non-streaming upstream returns zero-usage `HTTP 200` with no fallback after retry | same zero-usage validation persists after retry, but no later enabled candidate exists | Do not pass the upstream `200` body to the CLI; return aggregate proxy failure and keep one path failure sample for disable/rate-limit decisions |
...
- Good: a successful SSE response ends without body bytes once, then the same route path returns a valid stream on the bounded retry; only the successful final attempt is recorded and no fallback path is touched.
...
- Good: a Codex/OpenAI Responses stream containing `custom_tool_call` or `local_shell_call` output and a terminal `response.completed` / `response.incomplete` is forwarded without a locally injected empty-response error.
- Good: an Anthropic `event: error` or OpenAI `response.failed` event is delivered unchanged to the CLI and recorded with its upstream error type instead of being replaced by `missing_terminal_event`.
...
- Bad: keying successful-path affinity or local priority-hit highlights only by `routeRuleId`,
  `siteId/accountId/apiKeyId`, or `resolvedModel`, because a stale hit from another selected model
  or another matching route rule can keep routing Claude Code `GLM5.2` to ElySiver even after the
  user resets the current priority hit.
...
- `src/__tests__/unified-config-manager.test.ts`: assert priority-hit reset without `routeRuleId`
  writes `affinitySuppressedUntil` across matching route-rule paths, including the case where no
  existing path state matched.
- `src/__tests__/unified-config-manager.test.ts`: assert priority-hit reset without `resolvedModel`
  clears and suppresses every route-rule/resolved-model state for the same visible route channel and
  writes a channel-level fallback suppression state with no `routeRuleId` or `resolvedModel`.
...
- `src/__tests__/route-workbench-redesign.test.tsx`: assert the detail action bar shows `重置优先命中` before `恢复路径`, calls the reset action with channel identity and no `routeRuleId` / `resolvedModel`, and clears the local hit highlight after success.
...
- `src/__tests__/electron-fetch.test.ts`: assert raw-stream terminal/error listeners are registered before `data`, pending chunk callbacks settle before `end`, and an `aborted` response rejects immediately.
...
- `src/__tests__/route-proxy-service.test.ts`: assert an empty successful SSE body retries the same route path once before path failure/fallback is recorded.
...
- `src/__tests__/route-proxy-service.test.ts`: assert Anthropic `event: error` and OpenAI `response.failed` are preserved and recorded as upstream streaming errors without a duplicate missing-terminal event.
- `src/__tests__/route-proxy-service.test.ts`: assert `custom_tool_call`, `local_shell_call`, and other typed Responses output items plus `response.incomplete` are accepted as terminal consumable output.
...
- `src/__tests__/route-proxy-service.test.ts`: assert a non-streaming all-zero-usage `HTTP 200` is not returned to the CLI when no fallback candidate remains after retry.
...
| `stage: 'upstream'` + `error: 'upstream_streaming_error:*'` | Explicit upstream SSE failure | Proxy preserved an Anthropic/OpenAI failure event and recorded its type/code; this is not a missing-terminal failure |
...
| Upstream empty generation | often 200 stream then local fail | zero-byte SSE after bounded retry, truly empty `message` output, `empty_message`, `missing_message_start`, or `No choices in OpenAI response`; a typed Responses tool/reasoning output item is not empty |
...
- `malformed_streaming_response:*` with `stage: 'upstream'` means upstream content failed local
  protocol validation; verify the validator recognizes the current official event/output-item
  schema before assigning the failure to the provider or rewriting request format.
...
- Streaming validators (`malformed_streaming_response`, `incomplete_streaming_response`,
  `invalid_streaming_response`, zero-usage) are local interpretations of upstream bytes. Their
  error code alone does not prove an upstream semantic failure: compare the captured event types
  against the current protocol contract, preserve explicit upstream failure events, and only use
  `missing_terminal_event` when no success/incomplete/failure terminal event exists.
