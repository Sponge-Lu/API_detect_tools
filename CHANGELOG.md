...
### 新增
- **Route 工作台**：新增 `模型重定向`、`CLI 可用性`、`代理统计` 三个页面，配套路由规则、统一 API Key、服务启停、健康检查、CLI 探测历史与分析统计
- **路由配置持久化**：`config.routing` 新增 `server / rules / stats / health / modelRegistry / cliProbe / analytics` 配置域，并通过 `route:*` IPC 对外暴露
- **独立登录浏览器**：登录使用独立浏览器实例，不与检测浏览器池共用槽位，降低批量检测和登录流程互相抢占的概率
- **多账户自动刷新**：支持按账户粒度配置自动刷新开关和间隔，账户级配置优先于站点级
- **配置损坏自动恢复**：主配置读取失败时自动从备份恢复，新增结构校验与损坏副本保留，防止坏文件被静默清空
- **accessToken 自动补全**：登录后 token 缺失时自动创建，无需用户重试
...
### 修复
- **Cloudflare 拦截判断统一**：统一处理各类浏览器挑战页，替代原先的硬编码 HTML 检测
- **登录与检测互相干扰**：复用独立登录浏览器处理站点刷新，减少登录流程抢占检测资源导致的失败
- **回归测试补强**：新增路由、主题、站点统计、多账户与新 UI 相关测试，覆盖 v3 关键路径
...
### 新增
- **应用内更新下载面板**：支持在应用内查看更新说明、实时下载进度、取消下载，并在下载完成后直接触发安装
- **更新下载 IPC 通道**：新增 `update:start-download`、`update:cancel-download`、`update:install` 与 `update:download-progress` 事件

### 修复
- **Codex 配置模板兼容性**：适配 Codex 新配置格式，移除 `web_search_request`，改为 `web_search = "cached"`
- **CLI 配置编辑持久化**：仅保存用户手动编辑内容，未编辑时回退实时生成模板，避免旧内容覆盖新模板
- **构建稳定性**：修复 TypeScript 构建报错（含 Header 组件遗留属性问题）
...
### 优化
- **自定义 CLI 配置入口可见性**：CLI 状态面板入口从图标改为文字按钮（“自定义CLI配置”），降低识别成本
- **自定义 CLI 列表布局**：压缩卡片间距与按钮尺寸，提升同屏信息密度

### 修复
- **模型切换预览不同步**：修复自定义 CLI 配置编辑器切换模型后配置预览不刷新的问题
...
### 变更
- **LDC 刷新策略调整**：不再在刷新流程中导航到 `linux.do` 获取论坛积分，避免 Cloudflare 验证和 429 限流干扰
...
- **发布流程幂等性**：GitHub Actions 发布前自动删除同 tag 旧 Release，避免资产上传冲突
...
### 新增
- **一键签到所有可签到站点**：顶部新增批量签到按钮，可顺序执行所有可签到站点并汇总结果

### 优化
- **批量签到筛选逻辑**：自动跳过未认证站点、不支持签到站点、当天已签到站点
- **批量签到结果回写**：签到后同步刷新对应站点的余额和签到状态缓存
...
### 新增
- **早期 UI 原语重构**：首次引入完整的产品级原语体系
  - 新增早期设计系统 CSS 变量（颜色、字体、间距、圆角、阴影、模糊）
  - 新增按钮原语（Primary/Secondary/Tertiary 变体，支持 ref 转发）
  - 新增卡片原语（毛玻璃背景、展开/收起动画）
  - 新增输入/搜索输入原语（聚焦状态、密码显示切换）
  - 新增弹窗原语（缩放+淡入淡出动画、焦点陷阱）
  - 新增表格原语（分组样式、交错淡入动画）
  - 新增图标原语（统一 1.5px stroke-width、标准尺寸）
...
- **站点卡片样式**：移除渐变背景色，使用纯色背景 + 边框分隔，统一产品级卡片语义
- **二级卡片样式**：优化 SiteCardDetails 展开详情区域的卡片层次
- **CLI 操作按钮组**：统一圆角、边框、背景色和点击效果
...
- **站点卡片拖拽性能**：优化拖拽时的渲染性能
  ...
  - 只有受影响的卡片才会重新渲染
  - 卡片原语拖拽状态添加 will-change GPU 加速提示
- **原生感视觉优化**：全面优化 UI 组件的质感、层次与动效规范
